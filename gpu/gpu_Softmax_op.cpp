...
#include "Vulten_backend/ops/Softmax_op.h"
...
void SoftmaxOp_Compute(void* kernel, TF_OpKernelContext* ctx) {
  ...
  GET_INPUT_TENSOR("Softmax", input, 0, ctx, status)

  MAKE_OUTPUT_TENSOR("Softmax", output, 0, input_dims, T, ctx, status)
  ...
  softmax_op->run_op((vulten_ops::Data_type)T, input_tensor, output_tensor);
}
