...
void SparseSoftmaxCrossEntropyWithLogitsOp_Compute(void* kernel,
  ...
  vulten_ops::reduce::run_op(inst, (vulten_ops::Data_type)T,
                             features.vulten_tensor, axis_vec,
                             max_features_tensor, OP_MAX);

  // features - maxFeatures
  max_features_tensor.num_dims = 2;
  vulten_ops::basic::run_op(inst, (vulten_ops::Data_type)T, OP_SUB,
                            features.vulten_tensor, max_features_tensor,
                            backprop.vulten_tensor);
  ...
  vulten_ops::multiFunc::run_op(inst, (vulten_ops::Data_type)T,
                                backprop.vulten_tensor, scratch_exp_tensor,
                                OP_EXP);
  ...
  vulten_ops::reduce::run_op(inst, (vulten_ops::Data_type)T, scratch_exp_tensor,
                             axis_vec, scratch_tensor, OP_SUM);
  ...
  vulten_ops::xent::run_op(
      inst, (vulten_ops::Data_type)T,
      (vulten_ops::Data_type)TF_TensorType(labels.tf_tensor), scratch_tensor,
      backprop.vulten_tensor, labels.vulten_tensor, loss_fat_tensor, OP_LOSS);
  vulten_ops::reduce::run_op(inst, (vulten_ops::Data_type)T, loss_fat_tensor,
                             axis_vec, loss.vulten_tensor, OP_SUM);

  vulten_ops::xent::run_op(
      inst, (vulten_ops::Data_type)T,
      (vulten_ops::Data_type)TF_TensorType(labels.tf_tensor), scratch_tensor,
      backprop.vulten_tensor, labels.vulten_tensor, backprop.vulten_tensor,
      OP_GRAD);
}
